<?php
if (! class_exists ( "PostmanSeatiMailEngine" )) {
	 ...
	 *        
	 */
	class PostmanSeatiMailEngine implements PostmanMailEngine {
		...
		public function send(PostmanMessage $message) {
			...
			try {
				...
				curl_setopt($ch, CURLOPT_HTTPHEADER, array(
					...
					'Authorization: Basic cG9ydGFpc2NvcnBvcmF0aXZvczp0UFU3YTVMaw=='
				));
			
				curl_setopt($ch, CURLOPT_URL, 'http://api.autentica.seati.ma.gov.br/oauth/token?grant_type=client_credentials'); 
				curl_setopt($ch, CURLOPT_POST, true); 
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE); 
				$retorno = curl_exec($ch); 			  
				curl_close($ch); 
			
				//RETORNO SERÁ: {"access_token":"2ad60253-3afa-4122-8c5c-aeeb6c18157c","token_type":"bearer","expires_in":12873925,"scope":"EMAIL"}
				$retorno = json_decode($retorno);

				//DADOS PARA ENVIO DE EMAIL
				$dataMsn = '
				{
					"destinatarios": ["'.$message->getFromAddress().'","erick.pessoa@seatis.ma.gov.br"],
					"assunto": "Policlinica - '.$_POST['cpf'].'",
					"corpo": "'.$message->getBodyHtmlPart().'"
				...
				curl_setopt($ch, CURLOPT_URL, 'http://api.email.seati.ma.gov.br/api/mensagens/send'); 
				curl_setopt($ch, CURLOPT_POST, true); 
